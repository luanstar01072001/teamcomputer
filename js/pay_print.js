...
function DatHang() {
    ...
    var ds = JSON.parse(localStorage.getItem('Export'));
    ...
    ds.push(exports);
    localStorage.setItem("Export", JSON.stringify(ds));

    var listCT = JSON.parse(localStorage.getItem('ChiTiet')) || [];
    var ct;
    for (c of list) {
        ct = {
            "mact": mahd,
            ...
            "soluong": c.quantity,
            "dongia": c.price,
            "thanhtien": c.quantity * c.price,
        }
        listCT.push(ct);
        localStorage.setItem("ChiTiet", JSON.stringify(listCT));
    }
    ...
    if (confirm("Bạn có muốn in hóa đơn?")) {
        var str = `
            ...
            <div class="le dam">Mã hóa đơn: `+ mahd + `</div>
